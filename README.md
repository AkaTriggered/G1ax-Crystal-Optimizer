...
### 3. Modrinth Auto Update Checker (`UpdateChecker.java`)
On startup a silent background thread queries the Modrinth API for the latest release:
- If a newer version exists, a formatted chat notification appears **once** when you first join a world.
- Includes a direct download link and custom dev contact (`anupambuilds.shop`).
- Zero performance impact — daemon thread, non-blocking, 5s timeout.

> [!NOTE]
> Completely silent if you're already on the latest version. Never shows more than once per session.
...
└── util/
    ...
    └── UpdateChecker.java           (Background Modrinth update checker — no API key needed)
```
